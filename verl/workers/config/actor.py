...
class SelfDistillationConfig(BaseConfig):
    ...
    vcsd: bool = False
    vcsd_ctrl_mode: str = "none"
    vcsd_ctrl_image_key: Optional[str] = None
    vcsd_generic_prompt: str = "Describe this image in detail."
    vcsd_temperature: float = 2.0
    # Distillation target: "teacher" (plain EMA-teacher hi distribution) or "contrast"
    # (contrast-sharpened target softmax(lp_hi + α(lp_hi − lp_ctrl)) restricted to hi's
    # plausibility set
    # build_contrast_target in verl/trainer/ppo/vcsd.py).
    vcsd_target_mode: str = "teacher"
    # Contrast tilt strength α (offline pre-check: 0.5 conservative, 1.0 standard).
    vcsd_contrast_alpha: float = 1.0
    ...
    vcsd_contrast_anchor_coef: float = 1.0
    # Plausibility threshold β: candidate set = {w : p_hi(w) ≥ β max p_hi}.
    vcsd_contrast_beta: float = 0.1
    ...
    vcsd_contrast_exclude_token_ids: Optional[list[int]] = None
    vcsd_divergence_alpha: float = 0.0
    # Visual-token budgets for the offline degrade control (R_res).
    # downsample target area = vcsd_ctrl_low_tokens * 28 * 28 (Qwen3-VL patch=28),
    # matching unsup-opsd/vcsd `_LOW_TOKENS/_UP_TOKENS`.
    vcsd_ctrl_low_tokens: int = 256
    vcsd_ctrl_up_tokens: int = 1536
    ...
    def __post_init__(self):
        ...
        if self.vcsd_ctrl_mode not in valid_ra_ctrl_modes:
            raise ValueError(
                f"self_distillation.vcsd_ctrl_mode must be one of {valid_ra_ctrl_modes}, got {self.vcsd_ctrl_mode}"
            )
        if self.vcsd:
            if self.teacher_prompt_mode is not None:
                raise ValueError("self_distillation.vcsd requires teacher_prompt_mode=None.")
            if self.vcsd_ctrl_mode == "none":
                raise ValueError("self_distillation.vcsd requires vcsd_ctrl_mode != 'none'.")
            if self.vcsd_ctrl_mode in {"none", "degrade", "qvis", "black", "gaussnoise"} and not self.vcsd_ctrl_image_key:
                raise ValueError(
                    "self_distillation.vcsd_ctrl_image_key is required when vcsd=True and "
                    f"vcsd_ctrl_mode={self.vcsd_ctrl_mode!r}."
                )
            if self.vcsd_temperature <= 0:
                raise ValueError(f"self_distillation.vcsd_temperature must be positive, got {self.vcsd_temperature}")
            valid_vcsd_target_modes = ["teacher", "contrast"]
            if self.vcsd_target_mode not in valid_vcsd_target_modes:
                raise ValueError(
                    "self_distillation.vcsd_target_mode must be one of "
                    f"{valid_vcsd_target_modes}, got {self.vcsd_target_mode}"
                )
            if self.vcsd_target_mode == "contrast":
                if not self.full_logit_distillation or self.distillation_topk is not None:
                    raise ValueError(
                        "self_distillation.vcsd_target_mode='contrast' requires "
                ...
                # vcsd.py build_contrast_target reduces to log_softmax(lp_hi over plausible set).
                # Negative alpha is still rejected (would anti-tilt toward the ctrl branch).
                if self.vcsd_contrast_alpha < 0:
                    raise ValueError(
                        f"self_distillation.vcsd_contrast_alpha must be non-negative, got {self.vcsd_contrast_alpha}"
                    )
                # beta=0 disables the plausibility mask entirely (log(0)=-inf -> all tokens pass),
                # numerically safe in vcsd.py:387; allowed for the P13 no-mask ablation (2026-07-17).
                if not 0.0 <= self.vcsd_contrast_beta < 1.0:
                    raise ValueError(
                        f"self_distillation.vcsd_contrast_beta must be in [0,1), got {self.vcsd_contrast_beta}"
                    )
            if not 0.0 <= self.vcsd_divergence_alpha <= 1.0:
                raise ValueError(
                    "self_distillation.vcsd_divergence_alpha must be in [0,1], "
                    f"got {self.vcsd_divergence_alpha}"
                )
            if self.vcsd_ctrl_low_tokens <= 0:
                raise ValueError(
                    f"self_distillation.vcsd_ctrl_low_tokens must be positive, got {self.vcsd_ctrl_low_tokens}"
                )
            if self.vcsd_ctrl_up_tokens <= 0:
                raise ValueError(
                    f"self_distillation.vcsd_ctrl_up_tokens must be positive, got {self.vcsd_ctrl_up_tokens}"
                )
