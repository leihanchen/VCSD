...
from verl.trainer.ppo.vcsd import vcsd_kd_loss
        ...
        if self_distillation_enabled:
            ...
            if vcsd_enabled:
                if self_distillation_cfg.get("distillation_topk", None) is not None:
                    raise ValueError(
                        "VCSD requires full-vocab distillation; set self_distillation.distillation_topk=null."
                    )
                    ...
                    if self_distillation_enabled:
                        ...
                        if vcsd_enabled:
                            ...
                            ctrl_return_all_logps = (
                                self_distillation_cfg.get("vcsd_target_mode", "teacher") == "contrast"
                            )
                            with torch.no_grad():
                        ...
                        if vcsd_enabled:
                            if student_all_logps is None or teacher_all_logps is None or teacher_ctrl_log_prob is None:
                                raise ValueError(
                                    "VCSD requires student_all_logps, teacher_all_logps, and ctrl log probs."
                                )
                            vopd_loss, vopd_metrics = vcsd_kd_loss(
                                student_all_log_probs=student_all_logps,
                                teacher_all_log_probs=teacher_all_logps,
                                response_mask=response_mask,
                                self_distillation_mask=self_distillation_mask,
                                temperature=self_distillation_cfg.get("vcsd_temperature", 2.0),
                                divergence_alpha=self_distillation_cfg.get("vcsd_divergence_alpha", 0.0),
                                target_mode=self_distillation_cfg.get("vcsd_target_mode", "contrast"),
                                teacher_ctrl_all_log_probs=teacher_ctrl_all_logps,
                                contrast_alpha=self_distillation_cfg.get("vcsd_contrast_alpha", 1.0),
                                contrast_beta=self_distillation_cfg.get("vcsd_contrast_beta", 0.1),
                                contrast_anchor_coef=self_distillation_cfg.get("vcsd_contrast_anchor_coef", 1.0),
                                contrast_exclude_token_ids=self_distillation_cfg.get(
                                    "vcsd_contrast_exclude_token_ids", None
                                ),
                            )
                            vopd_metrics["vcsd/teacher_hi_logp_mean"] = (
                                (teacher_log_prob * response_mask).sum() / response_mask.sum().clamp(min=1.0)
                            ).detach().item()
                            vopd_metrics["vcsd/teacher_ctrl_logp_mean"] = (
