...
class RayPPOTrainer:
    ...
    def _maybe_build_self_distillation_batch(
        ...
        vcsd_enabled = bool(self_distillation_cfg.get("vcsd", False))
        ...
        if use_teacher_always_on_inputs:
            ...
            vcsd_ctrl_mode = self_distillation_cfg.get("vcsd_ctrl_mode", "none")
            vcsd_ctrl_image_key = self_distillation_cfg.get("vcsd_ctrl_image_key", None)
            ...
            if vcsd_enabled and vcsd_ctrl_mode not in {"noimg"}:
                if not vcsd_ctrl_image_key:
                    raise ValueError("VCSD control image key is required for this control mode.")
                if vcsd_ctrl_image_key not in batch.non_tensor_batch and vcsd_ctrl_image_key != student_image_key:
                    raise KeyError(f"VCSD control image key `{vcsd_ctrl_image_key}` not found in batch.non_tensor_batch")
            ...
            for i in range(batch_size):
                ...
                if vcsd_enabled:
                    raw_prompt_messages = list(batch.non_tensor_batch["raw_prompt"][i])
                    if vcsd_ctrl_mode == "noimg":
                        ctrl_messages = self._remove_images_from_messages(raw_prompt_messages)
                    else:
                        ctrl_images = get_sample_images(vcsd_ctrl_image_key, i)
                        if not self._teacher_images_available(ctrl_images):
                            raise ValueError(
                                f"VCSD control image key `{vcsd_ctrl_image_key}` is empty for sample {i}."
                            )
                        if vcsd_ctrl_mode == "black":
                            ctrl_images = self._make_black_images_like(ctrl_images)
                        if vcsd_ctrl_mode == "gaussnoise":
                            ctrl_images = self._make_noise_images_like(ctrl_images)
                        if vcsd_ctrl_mode == "qvis":
                            ctrl_prompt_messages = self._build_generic_visual_messages(
                                raw_prompt_messages,
                                self_distillation_cfg.get("vcsd_generic_prompt", "Describe this image in detail."),
                            )
            ...
            if vcsd_enabled:
                ...
                metrics[f"vcsd/ctrl_mode/{vcsd_ctrl_mode}"] = 1.0
            return DataProto.from_dict(
                tensors=tensors,
    ...
    def _get_gen_batch(self, batch: DataProto) -> DataProto:
        ...
        vcsd_ctrl_image_key = (
            self.config.actor_rollout_ref.actor.get("self_distillation", {}).get("vcsd_ctrl_image_key", None)
        )
        if vcsd_ctrl_image_key and vcsd_ctrl_image_key in batch.non_tensor_batch:
            reward_model_keys.add(vcsd_ctrl_image_key)
